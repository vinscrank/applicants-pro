...
CREATE INDEX idx_job_searches_searched_at ON job_searches (searched_at DESC);
...
CREATE INDEX idx_job_offers_search_id ON job_offers (search_id);
CREATE INDEX idx_job_offers_offer_id ON job_offers (offer_id);

CREATE TABLE job_applied_offers (
...
CREATE TABLE job_dismissed_offers (
    user_id INTEGER NOT NULL REFERENCES users (id) ON DELETE CASCADE,
    offer_id VARCHAR(64) NOT NULL,
