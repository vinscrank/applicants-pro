...
export function OffersTable({
  ...
  const seniorityLabels = useMemo(
    (): Record<Seniority, string> => ({
      entry: t('jobs.seniority.entry'),
      junior: t('jobs.seniority.junior'),
      mid: t('jobs.seniority.mid'),
      senior: t('jobs.seniority.senior'),
      lead: t('jobs.seniority.lead'),
  ...
  if (offers.length === 0) {
    return <div className="rounded-lg border border-dashed p-8 text-center text-muted-foreground">{t('jobs.tableEmpty')}</div>
  }
            ...
            {pageRows.map((row) => {
              ...
              return (
                <TableRow
                  ...
                  title={completed && appliedDate ? t('jobs.appliedOn', { date: appliedDate }) : undefined}
                >
