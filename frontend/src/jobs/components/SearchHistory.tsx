...
export function SearchHistory({ history, currentId, onSelect, loading }: Props) {
  ...
  const visitMeta = (entry: SearchSummary): string => {
    const parts = [
      t('jobs.historySheet.metaOffers', { count: entry.total_found }),
      t('jobs.historySheet.metaVerified', { count: entry.verified_count }),
    ]
    if (entry.maybe_count > 0) {
      parts.push(t('jobs.historySheet.metaMaybe', { count: entry.maybe_count }))
    }
    return parts.join(' · ')
  }

  if (history.length === 0) {
    return <p className="history-empty">{t('jobs.historySheet.empty')}</p>
  }
        ...
        <section key={category.key} className="history-category">
          <header className="history-category-header">
            ...
            <span className="history-category-count">
              {t('jobs.historySheet.visit', { count: category.visits.length })}
            </span>
          </header>
