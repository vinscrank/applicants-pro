  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="platform-form-dialog sm:max-w-md">
        <DialogHeader>
          <DialogTitle>
            {isArchived ? t('jobs.archivedTitle') : t('jobs.archivedMissing')}
          </DialogTitle>
            ...
            <div className="space-y-3 pt-1 text-left text-sm text-muted-foreground">
              {isArchived ? (
                <>
                  ...
                  <p>{t('jobs.archivedDescription')}</p>
                </>
              ) : (
                <p>
                  {t('jobs.archivedMissingDescription')}
                  {applicationId != null
                    ? t('jobs.archivedWithTracker')
                    : t('jobs.archivedNoTracker')}
                </p>
              )}
            </div>
          </DialogDescription>
